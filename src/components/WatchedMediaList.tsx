...
interface WatchedMediaListProps {
...
export function WatchedMediaList({
  items,
  contentType,
  query,
  minRating,
  ...
  onMinRatingChange,
  onGenreFilterChange,
}: WatchedMediaListProps) {
  const scoped = items.filter((item) => item.type === contentType);
  ...
  return (
    <section className="rounded-2xl border border-border/70 bg-card/60 p-4">
      <h2 className="font-display text-lg font-semibold">Meus títulos</h2>
      ...
      <ul className="mt-4 space-y-2">
        {filtered.length === 0 ? (
          <li className="text-sm text-muted-foreground">
            Nenhum título no histórico com esses filtros.
          </li>
        ) : (
          filtered.map((item) => (
            <li
