...
async function main(): Promise<void> {
  try {
    ...
    const popularOnNetflix = await discoverMoviesByWatchProvider({
      watchProviderIds: "8",
      watchRegion: "BR",
      page: 1,
    });
    const firstMovie = popularOnNetflix.results[0];

    console.log("TMDB connection ok");
    console.log(`Image base URL: ${configuration.images.secureBaseUrl}`);
    console.log(
      `Netflix movies (BR): ${String(popularOnNetflix.totalResults)}`,
    );
    ...
    if (error instanceof TmdbServiceError) {
      console.error(
        `TMDB smoke test failed [${error.code ?? "UNKNOWN"}]: ${error.message}`,
      );
      process.exitCode = 1;
      return;
    }
